...
from utils.resources import check_disk_space
import shutil
...
class ImageProcessHandler(FileSystemEventHandler):
    ...
    def _cleanup_old_data(self, min_free_gb: float = 10.0, max_age_days: int = 7):
        """清理旧数据以释放磁盘空间
        
        Args:
            min_free_gb: 目标最小可用空间（GB）
            max_age_days: 删除超过此天数的旧数据（默认7天）
        """
        try:
            self._log(f"🧹 开始清理旧数据（目标: {min_free_gb}GB 可用空间）...", 'info')
            
            # 需要清理的目录列表
            cleanup_dirs = [
                self.patches_dir,
                self.patches_keep_dir,
                self.patches_trash_dir,
                self.output_base_dir,
            ]
            
            # 如果存在，也清理缩放后的图像目录
            if hasattr(self, 'downscaled_wsi_dir') and self.downscaled_wsi_dir.exists():
                cleanup_dirs.append(self.downscaled_wsi_dir)
            ...
            for cleanup_dir in cleanup_dirs:
                ...
                for item in cleanup_dir.iterdir():
                    if item.is_dir():
                        try:
                            ...
                            subdirs.append((item, mtime, age_days))
                        except Exception:
                            continue
                ...
                # 删除旧目录直到达到目标空间
                for subdir, mtime, age_days in subdirs:
                    # 检查当前磁盘空间
                    try:
                        disk_info = check_disk_space(cleanup_dir, min_free_gb=0)
                        if disk_info.get('free_gb', 0) >= min_free_gb:
                            break
                    except Exception:
                        pass
                    ...
                    if age_days > max_age_days:
                        try:
                            ...
                            self._log(f"  ✓ 已删除: {subdir.name} ({age_days:.1f}天前, {dir_size / 1024 / 1024:.1f}MB)", 'info')
                        except Exception as e:
                            self._log(f"  ⚠️ 删除失败: {subdir.name} - {e}", 'warning')
            
            if total_freed > 0:
                freed_gb = total_freed / (1024 ** 3)
                self._log(f"✅ 清理完成，释放了 {freed_gb:.2f}GB 空间", 'info')
            else:
                self._log(f"ℹ️ 没有找到可清理的旧数据（保留最近 {max_age_days} 天的数据）", 'info')
                
        except Exception as e:
            self._log(f"⚠️ 清理旧数据时出错: {e}", 'warning')
    ...
    def process_image(self, image_path: Path, file_hash: str):
        ...
        try:
            # 检查输出目录的磁盘空间
            output_path = self.patches_dir if self.patches_dir.exists() else Path('/')
            disk_info = check_disk_space(output_path, min_free_gb=5.0)
            free_gb = disk_info.get('free_gb', 0)
            if free_gb < 10.0:
                self._log(f"⚠️ 警告：磁盘空间不足，仅剩 {free_gb:.2f}GB", 'warning')
                # 尝试清理旧数据
                self._cleanup_old_data(min_free_gb=10.0)
                # 再次检查
                disk_info = check_disk_space(output_path, min_free_gb=5.0)
                free_gb = disk_info.get('free_gb', 0)
                if free_gb < 5.0:
                    raise RuntimeError(f"磁盘空间严重不足，仅剩 {free_gb:.2f}GB，无法继续处理")
        except RuntimeError as e:
            self._log(f"❌ {e}", 'error')
            raise
        except Exception as e:
            self._log(f"⚠️ 检查磁盘空间失败: {e}，继续处理...", 'warning')
            ...
            for x_idx in range(num_patches_x):
                ...
                try:
                    ...
                    if e.errno == 28:  # No space left on device
                        self._log(f"\n❌ 磁盘空间不足，无法保存 patch {patch_count}", 'error')
                        # 尝试清理旧数据
                        try:
                            self._cleanup_old_data(min_free_gb=10.0)
                            # 重试保存
                            patch_pil.save(save_path, 'PNG')
                            self._log(f"✅ 清理后成功保存 patch {patch_count}", 'info')
                        except Exception as cleanup_error:
                            self._log(f"❌ 清理磁盘空间失败: {cleanup_error}", 'error')
                ...
                try:
                    ...
                    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
                        ...
                        for i in range(patch_count):
                            ...
                            orig_y_end = int((y_r[i] + self.patch_size) * scale_factor)
                            writer.writerow([filename, orig_x_start, orig_y_start, orig_x_end, orig_y_end])
                    self._log(f"✅ 清理后成功保存 CSV 文件", 'info')
                except Exception as cleanup_error:
                    self._log(f"❌ 清理磁盘空间失败: {cleanup_error}", 'error')
                    raise RuntimeError("磁盘空间不足，无法继续处理")
            else:
