...
class NewModel(LabelStudioMLBase):
    ...
    def setup(self):
        ...
        self.transform = T.Compose([
            T.Resize((640, 640)),  # RT-DETR通常使用固定尺寸输入
            T.ToTensor(),
        ...
        try:
            ...
            if self.device.type == 'cuda':
                try:
                    ...
                    if "Expected all tensors to be on the same device" in str(cuda_test_error):
                        print("⚠️  警告: TorchScript模型似乎是在CPU上导出的，无法在CUDA上运行")
                        print("   建议: 使用修复后的export_pt.py在CUDA设备上重新导出模型")
                        print("   当前: 将切换到CPU模式")
                        self.device = torch.device('cpu')
                        self.force_cpu = True
                        # 重新加载到CPU
    ...
    def postprocess(self, outputs, original_size, image_tensor_size):
        ...
        for box, score, label in zip(boxes, scores, labels):
            # 解析边界框坐标
            # RT-DETR torchscript导出通常输出归一化的[x1, y1, x2, y2]格式
            if len(box) == 4:
                x1, y1, x2, y2 = box
    ...
    def predict(self, tasks: List[Dict], context: Optional[Dict] = None, **kwargs) -> ModelResponse:
        ...
        for task_idx, task in enumerate(tasks):
            ...
            try:
                ...
                # 推理
                with torch.no_grad():
                    orig_target_sizes_tensor = None
                    if getattr(self, "_requires_orig_target_sizes", False):
                    ...
                    try:
                        if orig_target_sizes_tensor is not None:
                            outputs = self.model(image_tensor, orig_target_sizes_tensor)
                        else:
                            # 尝试直接调用（单参数）
                            outputs = self.model(image_tensor)
                    except RuntimeError as e:
                        if "orig_target_sizes" in str(e):
                            # 模型需要orig_target_sizes参数
                            print(f"模型需要orig_target_sizes参数")
                            ...
                            try:
                                outputs = self.model(image_tensor, orig_target_sizes)
                            except RuntimeError as dev_e:
                                # 处理TorchScript内部常量强制在CPU导致的设备不一致问题
                                if "Expected all tensors to be on the same device" in str(dev_e):
                                    print("检测到设备不一致（cuda 与 cpu），切换到 CPU 推理并重新加载模型")
                                    self.force_cpu = True
                                    self._load_model(self.model_path, force_cpu=True)
                                    image_tensor = image_tensor.cpu()
                                    orig_target_sizes = orig_target_sizes.cpu()
                                    outputs = self.model(image_tensor, orig_target_sizes)
                                    orig_target_sizes_tensor = orig_target_sizes
                                else:
                                    raise
                        elif "Expected all tensors to be on the same device" in str(e):
                            # 单参数前向即出现设备不一致，直接回退到CPU重试（某些模型内部常量被固定到CPU）
                            print("检测到设备不一致（cuda 与 cpu），切换到 CPU 推理并重新加载模型")
                            self.force_cpu = True
                            self._load_model(self.model_path, force_cpu=True)
                            image_tensor = image_tensor.cpu()
                            if orig_target_sizes_tensor is None:
                                orig_target_sizes_tensor = torch.tensor(
                                    [[original_size[1], original_size[0]]],
                                    dtype=torch.float32
                                )
