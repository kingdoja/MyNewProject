...
class BaseSolver(object):
    ...
    def load_tuning_state(self, path: str,):
        ...
        # TODO hard code
        # 兼容没有 EMA 或不同权重结构的检查点
        params = None
        if 'ema' in state and state['ema'] is not None and isinstance(state['ema'], dict) and 'module' in state['ema']:
            params = state['ema']['module']
        elif 'model' in state and isinstance(state['model'], dict):
            params = state['model']
        else:
            # 直接尝试使用整个state作为权重（适配纯state_dict）
            if all(isinstance(v, torch.Tensor) for v in state.values()):
                params = state

        if params is None:
            raise RuntimeError('Unsupported checkpoint format for tuning. Expected keys in ["ema.module", "model"] or a pure state_dict.')
