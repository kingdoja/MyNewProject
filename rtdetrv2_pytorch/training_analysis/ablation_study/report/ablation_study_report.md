...
实验结果表明，完整模型（包含所有三个机制）相比基线模型在AP@0.50:0.95指标上提升了 **13.84%**，从 **0.185** 提升到 **0.323**。
...
Mamba机制单独使用时的AP@0.50:0.95为 **0.188**，相比基线提升了 **0.31%**。
这表明Mamba机制在序列建模方面的优势能够有效提升模型对细胞形态特征的捕获能力。

#### 5.1.2 HIFM机制

HIFM机制单独使用时的AP@0.50:0.95为 **0.200**，相比基线提升了 **1.55%**。
分层特征融合能够更好地整合多尺度信息，对检测不同尺寸的细胞有显著帮助。

#### 5.1.3 DSCA机制

DSCA机制单独使用时的AP@0.50:0.95为 **0.296**，相比基线提升了 **11.07%**。
可变形注意力机制能够自适应地关注关键区域，提高模型对细微特征的敏感性。

### 5.2 组件组合效果分析

完整模型（Mamba + HIFM + DSCA）达到了最佳的AP@0.50:0.95性能 **0.323**，相比基线提升了 **13.84%**。
...
2. **组合使用效果最佳**：完整模型（包含所有三个机制）达到了最佳性能，AP@0.50:0.95为 **0.323**，相比基线提升了 **13.84%**。
