...
# 添加项目路径 - 修复：需要指向 rtdetrv2_pytorch 目录
script_dir = Path(__file__).parent  # training_analysis/
rtdetrv2_pytorch_dir = script_dir.parent  # rtdetrv2_pytorch/
...
def main():
    # 配置参数 - 针对 cancer_detection1.yml / split_dataset_aug 数据集
    ...
    # 创建输出目录（在training_analysis/output/下）
    script_dir = Path(__file__).parent
    output_dir_path = script_dir / "output" / output_dir
    output_dir_path.mkdir(parents=True, exist_ok=True)
    output_dir = str(output_dir_path)
    
    # 尝试从验证集标注文件加载类别
    val_annotation_file = os.path.join(data_root, "val", "annotations", "instances_val.json")
    annotation_file = val_annotation_file if os.path.exists(val_annotation_file) else None
